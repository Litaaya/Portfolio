...
- `Untracked files` nghĩa là git chưa lưu vết nó trong dự án, nó ở đây là folder `git` nằm trong folder `posts` (cái này là tên folder mình đặt thôi, không thật sự liên quan tới git), nếu muốn lưu vết phải `git add` nó (cái này sẽ nói ở phía sau, `status` chỉ là để coi trạng thái thôi).
...
- `up to date ...`: phần này nghĩa là code của mình đã trùng với phiên bản hiện tại trên `origin/main`, nghĩa là cái github của mình.
...
Ngoài ra có thể so sánh sự khác biệt giữa hai nhánh 1 và 2 với câu lệnh cuối.
...
`git restore`: nó là `Ctrl + Z` đấy, `git restore <file>` reset hoàn toàn file đó, hoặc `git restore --staged <file>` để bỏ cái file khỏi staging area, đưa nó về trạng thái chưa `git add`, còn nếu không thì xài `git restore .` là sẽ hủy toàn bộ các file trong dự án (những file chưa `git add`).
...
Tại một thời điểm nhất định, chúng ta cần tạo một branch phụ để làm việc, `-b` sẽ giúp ta skip qua 2 bước, tạo và chuyển qua nhánh mới.
...
2. Push nhánh phụ lên `origin`, tức github, để có nhánh `origin/<nhánh phụ>`, merge cái nhánh phụ đó vào `origin/main` trên github, pull down cái nhánh `main` origin xuống nhánh `main` ở local.
...
- Cái số 2: Tôi đẩy ý tưởng, tức nhánh phụ, lên cloud để hệ thống/đồng nghiệp kiểm tra. Khi mọi thứ OK mới gộp vào `main`, rồi kéo bản chuẩn về máy.
...
Với cách 2, ta sẽ push cái nhánh phụ lên `origin`, tức github, để github tạo nhánh phụ, xong mình sẽ thao tác trên github để pull request, tạo yêu cầu merge để quản lý dự án coi xem nhánh này có ảnh hưởng gì tới nhánh `main` không. Hai dòng cuối là sau khi nhánh của mình được chấp thuận merge vào `main`, ta sẽ kéo cái `origin/main` về local để đồng bộ với dự án.
